...
public class OkHttpDownloader extends Downloader {
	...
	public String execute(Request request) throws IllegalArgumentException, IOException {
		...
		// Handle cache
		int maxCacheAge = request.skipCache ? 0 : httpCacheMaxAge;

		// Build proper URL
		String requestUrl = toUrl(request);

		// Build the OkHttp request
		com.squareup.okhttp.Request httpRequest =
			new com.squareup.okhttp.Request.Builder().addHeader("Cache-Control", "public, max-age=" + maxCacheAge).url(requestUrl).build();
	...
	public String toUrl(Request request) {
		...
		if (request.individual) {
			...
			if (request.search != null) url += "?s=" + Uri.encode(request.search);
			if (request.page > 1) url += (request.search == null ? "?paged=" : "&paged=") + String.valueOf(request.page);
		}
